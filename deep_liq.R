...
attrition_df_df <-  read.csv("F:/deepLearning/ibm-hr-analytics-employee-attrition_df-performance/attrition_df.csv")


toYesNO  <- function(dataframe){
  
  
}
#attrition_df
#mutate new coluimn attrition_df_ to hold 0 and 1 for attrition_df and the ndeleting it 
attrition_df_df <- attrition_df %>% mutate(attrition_df_ = ifelse(attrition_df =="Yes",1,0)) ; attrition_df
